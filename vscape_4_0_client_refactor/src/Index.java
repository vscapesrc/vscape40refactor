...
final class Index {
   ...
   public final synchronized byte[] decompress(int indexFile) {
      try {
         ...
         if((var3 = ((buffer[3] & 255) << 16) + ((buffer[4] & 255) << 8) + (buffer[5] & 255)) > 0 && (long)var3 <= this.data.length() / 520L) {
   ...
   private synchronized boolean put(boolean exists, int position, int length, byte[] bytes) {
      try {
         ...
         if(!exists) {
            ...
            while(true) {
               if(var5 >= 6) {
                  if((var7 = ((buffer[3] & 255) << 16) + ((buffer[4] & 255) << 8) + (buffer[5] & 255)) <= 0 || (long)var7 > this.data.length() / 520L) {
                     return false;
                  }
                  break;
               }
